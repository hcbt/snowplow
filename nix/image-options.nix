...
# The runner-image OPTIONS, and the mapping from an option set to a built
# image.
#
# Kept apart from any module system so the same definition can back both
# `flakeModules.default` (flake-parts) and `devenvModules.default` (a plain
# file a devenv project imports). The two wrappers differ in three lines.
{ lib, mkImage }:
let
